const posts = [
  ...
  interface INormalizedData {
    byId: {[id: string]: IObjectData},
    allIds: string[]
  }
  
  const normalizeData = (unnormalizedData: IObjectData[]): INormalizedData => {
    let normalizedData: INormalizedData = {
        byId: {},
        allIds: []
    }

    unnormalizedData.map(({id, title, body}) => {
        normalizedData.byId[id] = {id, title, body}
